...
def Xnet(input_size, encoder_filters=(16, 32, 64, 128, 256), attention=True):
    ...
    if attention:
        g1 = UnetGatingSignal(conv5, is_batchnorm=True, name='g1')
        attn1 = AttnGatingBlock(conv4, g1, encoder_filters[3], '_1')
        up3_1 = concatenate([Transpose2D(conv5, encoder_filters[3]), attn1], axis=-1, name='up3_1')
    else:
        up3_1 = concatenate([Transpose2D(conv5, encoder_filters[3]), conv4], axis=-1, name='up3_1')

    conv6 = UnetConv2D(up3_1, encoder_filters[3], is_batchnorm=True, name='conv6')
    if attention:
        g2 = UnetGatingSignal(conv6, is_batchnorm=True, name='g2')
        attn2 = AttnGatingBlock(conv3, g2, encoder_filters[2], '_2')
        up2_2 = concatenate([Transpose2D(conv6, encoder_filters[2]), attn2], axis=-1, name='up2_2')
    else:
        up2_2 = concatenate([Transpose2D(conv6, encoder_filters[2]), conv3], axis=-1, name='up2_2')
