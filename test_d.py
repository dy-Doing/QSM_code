...
def find_numfromstr(str):
    ...
    num = int(f[0])
    return num
def show_output(y1_in,y1_out,y2_in,y2_out, y_image):
    for h in range(n):
        if np.sum(label_test[h, :, :, 0]) != label_test.shape[1]*label_test.shape[2] \
            ...
            plt.subplot(121)
            plt.imshow(y_image[:, :, h], cmap='gray', vmin=0.19, vmax=0.38)
            plt.contour(y1_in[:, :, h], linewidths=0.19, cmap=ListedColormap('red'))
            plt.contour(y1_out[:, :, h], linewidths=0.19, cmap=ListedColormap('yellow'))
            plt.contour(y2_in[:, :, h], linewidths=0.19, cmap=ListedColormap('red'))
            plt.contour(y2_out[:, :, h], linewidths=0.19, cmap=ListedColormap('yellow'))
            plt.show()
...
save_cnn =r'D:/htfg_all/CNN'
log_dir = r'C:/Users/ZHOUFF/PycharmProjects/htfg_tf2/log_all/log_attUnet_d/att_Unet32_4_d_2/'
model_dir = r'C:/Users/ZHOUFF/PycharmProjects/htfg_tf2/model_all/model_attUnet_d/att_Unet32_4_d_2.h5'
model = load_model(log_dir + 'ep345-loss0.005-val_loss0.003.h5', custom_objects={'tversky_loss': tversky_loss,'Dice': Dice_train})
# model = load_model(model_dir, custom_objects={'tversky_loss': tversky_loss,'Dice': Dice_train})
model.summary()

for j in np.arange(81,101):
        # x=j
    # if j !=82:
        #     x = j+1
        # else:
    ...
    # else:
        print(j)
    #     print(x)
        file = h5py.File(r'C:/Users/ZHOUFF/PycharmProjects/htfg/data/d_h5/test_3D_d_original_' + str(j) + '.h5', 'r')
        ...
        #     if np.sum(label_test[h, :, :, 0]) != label_test.shape[1]*label_test.shape[2]:
        #         plt.imshow(data_test[h,:, :, 1], cmap='gray')
        #         # plt.contour(label_test[h, :, :, 0], linewidths=0.19, cmap=ListedColormap('red'))
        #         plt.contour(label_test[h, :, :, 1], linewidths=0.19, cmap=ListedColormap('red'))
        #         plt.contour(label_test[h, :, :, 2], linewidths=0.19, cmap=ListedColormap('yellow'))
        #         plt.show()
        ...
        # y_image = (y_image - np.min(y_image))/(np.max(y_image)-np.min(y_image))
        # show2 = Imshow3DArray(y_image, ROI=[y1_in, y1_out])
        #todo# write_output_as_nii(test_path, y1_out)
        # show_output(y1_in,y1_out,y2_in,y2_out, y_image)
        ...
        # print('VOE1=%f'%(Voe1))
        # print('RVD1=%f'%(Rvd1))
        # print('Precision1=%f' % (precision1))
        # print('Recall1=%f' % (recall1))
        # print('Iou1=%f' % (Iou1))
        # print('VOE2=%f' % (Voe2))
        # print('RVD2=%f' % (Rvd2))
...
print('Dice_std=%f' % (std_dice_1))
print((np.sum(dice_all1) / 20))
# plot_hist_of_dice(dice_all1)
dice_all2 = np.reshape(dice_all_2, [-1, 1])
std_dice_2 = np.std(dice_all2)
print('Dice_std=%f' % (std_dice_2))
print((np.sum(dice_all2) / 20))
