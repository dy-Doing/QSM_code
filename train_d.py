...
from unet_ATT import Xnet
from Unet import Unet_d
import numpy as np
import h5py
...
def inputcase2(dir1, num_str, num_end, train_batch_1, filename, data_name, label_name):
    while True:
        ...
        for j in range(label_for_train.shape[0]):
            image_for_train[j, :, :, 0:3],label_for_train[j, :, :, 0:3] = \
                dataAug(image_for_train[j, :, :, :3],label_for_train[j, :, :, 0:3])
        plt.imshow(image_for_train[0, :, :, 1],cmap='gray')
        plt.show()
        ...
        # for h in range(image_for_train.shape[0]):
        #         plt.imshow(image_for_train[h,:, :, 1], cmap='gray')
        #         # plt.contour(label_test[h, :, :, 0], linewidths=0.19, cmap=ListedColormap('red'))
        #         plt.contour(label_for_train[h, :, :, 1], linewidths=0.19, cmap=ListedColormap('red'))
        #         plt.contour(label_for_train[h, :, :, 2], linewidths=0.19, cmap=ListedColormap('yellow'))
        #         plt.show()
        yield (image_for_train, [label_for_train, label_down_3])


dir1 = r'C:/Users/ZHOUFF/PycharmProjects/htfg/data/d_h5/'
log_dir = r'C:\Users\ZHOUFF\PycharmProjects\htfg_tf2\log_all\log_attUnet_d\Unet16_4_d_eeee'
model_dir = r'C:/Users/ZHOUFF/PycharmProjects/htfg_tf2/model_all/model_attUnet_d/Unet16_4_d_eeee.h5'
# train_number = 1162
# vali_number = 269
train_number = 638
vali_number = 92
...
model = Unet_d([patch_row, patch_col, 3])
loss = {
        # 'pred2':tversky_loss,
        'pred3': tversky_loss,
        'final': tversky_loss}
...
model.compile(optimizer=Adam(lr=1e-4), loss=loss, loss_weights=loss_weights, metrics=dice)

# 载入数据

trainGenerator = inputcase2(dir1, 0,1, train_batch, 'train_3D_d_all_', 'train_data', 'train_label')
validationGenerator = inputcase2(dir1,0, 1, vali_batch, 'validation_3D_d_all_', 'vali_data', 'vali_label')
